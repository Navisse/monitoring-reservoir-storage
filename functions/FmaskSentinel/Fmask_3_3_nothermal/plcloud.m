function [zen,azi,ptm,WT,Snow,Cloud,Shadow,dim,ul,...
...
n_B2=dir('*B2*');
[~,R] = geotiffread(n_B2.name);
InfoB = geotiffinfo(n_B2.name);

NDVI=(data(:,:,4)-data(:,:,3))./(data(:,:,4)+data(:,:,3));
NDSI=(data(:,:,2)-data(:,:,5))./(data(:,:,2)+data(:,:,5));
MNDWI=(data(:,:,5)-data(:,:,2))./(data(:,:,5)+data(:,:,2));

NDVI((data(:,:,4)+data(:,:,3))==0)=0.01;
NDSI((data(:,:,2)+data(:,:,5))==0)=0.01;
MNDWI((data(:,:,5)+data(:,:,2))==0)=0.01;

CoordRefSysCode = ['EPSG:326',num2str(InfoB.Zone)];
geotiffwrite('NDVI_tmp',NDVI,R,'CoordRefSysCode',CoordRefSysCode)
geotiffwrite('MNDWI_tmp',MNDWI,R,'CoordRefSysCode',CoordRefSysCode)
clear n_B2 R InfoB MNDWI CoordRefSysCode

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%saturation in the three visible bands
satu_Bv=satu_B1+satu_B2+satu_B3>=1;
clear satu_B1; % clear satu_B;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Basic cloud test
idplcd=NDSI<0.8&NDVI<0.8&data(:,:,6)>300;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Snow test
...
if ptm <= 0.1 % no thermal test => meanless for snow detection (0~1)
    ...
    wfinal_prob=100*Brightness_prob + 100*Thin_prob; % cloud over water probability
...
%    wclr_max=50;% fixed threshold (water)
    ...
    final_prob=100*Vari_prob + 100*Thin_prob; % cloud over land probability
